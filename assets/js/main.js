...
document.addEventListener("DOMContentLoaded", function () {
	...
	const observer = new IntersectionObserver((entries) => {
		entries.forEach((entry, index) => {
			...
			// Keep content visible once shown - removed the else block that hides it
		});
...
// Replaced mouseenter with IntersectionObserver for features section
document.addEventListener("DOMContentLoaded", function () {
	...
	const observer = new IntersectionObserver((entries) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) {
				...
				box.style.animation = `fadeInUp 0.8s ease-in-out ${index * 0.2}s forwards`;
				observer.unobserve(box); // Only animate once
			}
		});
	}, { threshold: 0.1 });
	...
	const observer = new IntersectionObserver((entries, observer) => {
		entries.forEach((entry, index) => {
			if (entry.isIntersecting) {
				...
				// Removed the code that hides the text after 2 seconds
			} else {
				// Optional: Remove animated class if you want it to re-animate on scroll up/down
				// But strictly, we should probably keep it visible once shown to avoid empty boxes
